...
import org.wk.services.RhinoService;
...
public class WelcomePanel extends AbstrPanel{
	...
	public WelcomePanel(JFrame frame, String name, File file){
		super(frame);
		this.inputFile = file;
		this.name = name;
	...
	private void setUpLayout() {
		...
		this.add(getResponseJsPanel(), BorderLayout.CENTER);
	...
	private JPanel getResponseJsPanel(){
		
		panel = new JPanel();
		
		Object[] args = new Object[]{name};
		String jsFunctionName = RhinoService.JS_FUNCTION_NAME;
		String message = (String)rhinoService.runJsFile(inputFile, jsFunctionName, args);
		
		panel.add(new JLabel("Response from js: " + message));
		return panel;
