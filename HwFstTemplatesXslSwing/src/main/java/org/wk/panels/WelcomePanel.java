...
public class WelcomePanel extends AbstrPanel{
	...
	private XmlService xslService;
	
	
	public WelcomePanel(JFrame frame){
		super(frame);
		xslService = new XmlService();
	...
	private JPanel getFilePanel(){
		...
		panel = new JPanel();
		panel.add(new JLabel("The name was taken from file: " + XmlService.XML_OUTPUT_FILE));
	...
	private String readNameFromXmlFile(){
		...
		UserEntity user = (UserEntity) xslService.readObjectFromFile(UserEntity.class, new File(XmlService.FILE_PATH + XmlService.XML_OUTPUT_FILE));
		return user.getName();
